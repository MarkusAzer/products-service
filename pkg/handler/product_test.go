...
func TestBookmarkIndex(t *testing.T) {
	...
	service := productMock.NewMockUseCase(controller)
	r := mux.NewRouter()
	...
	service.EXPECT().Create(gomock.Any()).Return(ID, nil)
	...
	http.Post(ts.URL+"/v1/products/command/create", "application/json", strings.NewReader(payload))
